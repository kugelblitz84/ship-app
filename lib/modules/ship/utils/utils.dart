...
class ShipStatementUtil {
  ...
  static _LedgerBuildResult _buildLedger(
    ...
    for (final tx in transactions) {
      if (_isTripTransaction(tx)) {
        if (_isStandaloneTripCreditTransaction(tx, tripById)) {
          items.add(
            _LedgerRawItem(
              dateRaw: tx.date,
              description: _standaloneTripTransactionDescription(tx),
  ...
  static bool _isStandaloneTripCreditTransaction(
    tx_models.TransactionModel tx,
    Map<String, trip_models.TripModel> tripById,
  ) {
    if (!_isTripTransaction(tx)) return false;
    ...
    return !tripById.containsKey(tripId);
  }

  static String _standaloneTripTransactionDescription(
    tx_models.TransactionModel tx,
  ) {
    final from = _safe(tx.tripFrom);
    final to = _safe(tx.tripTo);
    final company = _safe(tx.companyAndShipInfo.companyName ?? 'N/A');
    final shipPart = _optionalShipLine(tx.companyAndShipInfo.shipName);
    return _pdfText('Trip bill: $from - $to\nCompany: $company$shipPart');
  }
